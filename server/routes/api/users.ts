...
export default function (app: Express) {
  app
    ...
    .post(async (req, res) => {
      const { body } = req;
      if (!("username" in body && "password" in body && "invite" in body))
        return res.status(400).send({
          message:
            "Request must have properties `username`, `password`, and `invite`.",
        });
      ...
      if (!invite)
        return res.status(401).send({ message: "Invalid invite code. " });
      if (!invite.open)
        return res.status(401).send({ message: "Invite is no longer open." });
    ...
    .patch(async (req, res) => {
      ...
      if (!("username" in body.auth && "password" in body.auth))
        return res.status(400).send({
          message:
            "Request must have properties `auth.username` and `auth.password`.",
        });
